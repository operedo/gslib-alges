...
c
      ...
      subroutine setrot_opt01(ang1,ang2,ang3,anis1,anis2,ind,MAXROT,
     +                        rotmat)
c-----------------------------------------------------------------------
c
c              Sets up an Anisotropic Rotation Matrix
...
c      real*8    rotmat(MAXROT,3,3),afac1,afac2,sina,sinb,sint,
      real*8    rotmat(3,3,MAXROT),afac1,afac2,sina,sinb,sint,
     +          cosa,cosb,cost
c
c Converts the input angles to three angles which make more
c  mathematical sense:
...
c
      afac1 = 1.0 / dble(max(anis1,EPSLON))
      afac2 = 1.0 / dble(max(anis2,EPSLON))
c      rotmat(ind,1,1) =       (cosb * cosa)
c      rotmat(ind,1,2) =       (cosb * sina)
c      rotmat(ind,1,3) =       (-sinb)
c      rotmat(ind,2,1) = afac1*(-cost*sina + sint*sinb*cosa)
c      rotmat(ind,2,2) = afac1*(cost*cosa + sint*sinb*sina)
c      rotmat(ind,2,3) = afac1*( sint * cosb)
c      rotmat(ind,3,1) = afac2*(sint*sina + cost*sinb*cosa)
c      rotmat(ind,3,2) = afac2*(-sint*cosa + cost*sinb*sina)
c      rotmat(ind,3,3) = afac2*(cost * cosb)

      rotmat(1,1,ind) =       (cosb * cosa)
      rotmat(2,1,ind) =       (cosb * sina)
      rotmat(3,1,ind) =       (-sinb)
      rotmat(1,2,ind) = afac1*(-cost*sina + sint*sinb*cosa)
      rotmat(2,2,ind) = afac1*(cost*cosa + sint*sinb*sina)
      rotmat(3,2,ind) = afac1*( sint * cosb)
      rotmat(1,3,ind) = afac2*(sint*sina + cost*sinb*cosa)
      rotmat(2,3,ind) = afac2*(-sint*cosa + cost*sinb*sina)
      rotmat(3,3,ind) = afac2*(cost * cosb)
